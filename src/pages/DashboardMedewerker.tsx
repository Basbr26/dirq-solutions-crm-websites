...
const statusConfig: Record<string, { label: string; variant: 'destructive' | 'default' | 'secondary' }> = {
  ...
  herstel_gemeld: { label: 'Herstel Gemeld', variant: 'default' },
...
export default function DashboardMedewerker() {
  ...
  if (loading) {
    return (
      <AppLayout title="Mijn Overzicht">
  ...
  return (
    <AppLayout title="Mijn Overzicht" subtitle={format(new Date(), 'EEEE d MMMM yyyy', { locale: nl })}>
      <PullToRefresh onRefresh={loadActiveCase} className="h-[calc(100vh-4rem)] md:h-auto md:overflow-visible">
        <div className="container mx-auto px-4 sm:px-6 py-4 sm:py-8 max-w-4xl">
                ...
                <Card className="shadow-dirq mb-4 sm:mb-6">
                  <CardHeader className="px-4 sm:px-6 pt-4 sm:pt-6">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-0">
                      ...
                      <Badge variant={statusConfig[activeCase.case_status]?.variant || 'secondary'} className="text-xs self-start sm:self-auto">
                        {statusConfig[activeCase.case_status]?.label || activeCase.case_status}
                      </Badge>
                    </div>
                  </CardHeader>
